...
public class SchedulePane extends Mode {
	...
	private Label title, dateTitle, dateLbl;
	...
	public SchedulePane(Data data) {
		super(data);	
		searchFor(new Date()); //view today's rides by default
	}
	...
	public void addContent() {
		...
		dateTitle = new Label("Date");
		dateLbl = new Label(); //Eg, Mar 21, 2021 
	...
	public void addEvents() {
		...
		prevDate.getKeyFrames().add(new KeyFrame(Duration.millis(200), k->{
			incrementDate(-1);
			if (prevDate.getRate() < 10)
				prevDate.setRate(prevDate.getRate()+0.1);
		}));
		...
		nextDate.getKeyFrames().add(new KeyFrame(Duration.millis(200), k->{
			incrementDate(1);
			if (nextDate.getRate() < 10)
				nextDate.setRate(nextDate.getRate()+0.1);
		}));
	...
	public void searchFor(Date date) {
		...
		this.ride_results.addAll(data.searchRide(date));
	...
	private void addCell(int row, int col, String text) {
		...
		if (row == 0) { //bind the corresponding header title to this cell in row 0
			int c = 0;
			Label title = null;
			Iterator<Entry<String, Label>> iterator = field_labels.entrySet().iterator();
	...
	private void addLayout(int col, Label cell) {
		if (field_names[col].equals("Driver") && cell.getText().equals(Driver.nullDriver.getFullName())) {
